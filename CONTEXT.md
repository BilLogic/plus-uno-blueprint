...
**entrance**, **boot**, **revealStage** and **hold** are defined in the
template's `CONTEXT.md`, under the same heading. They belong to the shell and
the canvas, which this deployment imports rather than owns, so the definitions
live where the code does and are not restated here. The template also carries
the ADR that says how the two clocks meet.
