-- public.search_blueprint v4 — include.
--
-- v3 answered "which rows". This adds "…and what hangs off them". uno-bot
-- pays up to FIVE round trips for one Slack question — search, then edges,
...
-- edges and you get 15 cells. They carry the same 16 columns, tagged
-- kind = 'edge' | 'finding' | 'slice', with the structured payload in `links`.
--
-- Measured on production, Warm-Up at match_count 60:
--   60 cells + 38 edges + 3 findings + 9 slices, in ONE call.
--
-- Defaults to '{}', so every v3 caller is byte-identical. Verified against a
-- baseline taken before the change: 'host key' 1 row / total 1;
-- filter_scenario Warm-Up 5 rows / total 76; granularity
-- ['phase','scenario','path'] 66 rows / total 66.
