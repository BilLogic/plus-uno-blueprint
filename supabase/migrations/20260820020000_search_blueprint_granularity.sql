-- public.search_blueprint v3 — granularity.
--
-- v2 answered "which cells", and only cells: `kind` was the literal
-- 'cell'::text on every row. That single hidden assumption is why two
-- consumers independently hand-rolled the same structural index —
-- uno-bot's fetchBlueprintIndex (phases -> service_scenarios -> paths,
-- ~210 lines with its cache and flag) and the canvas agent's
-- list_scenarios (phases -> service_scenarios). Neither could ask this
-- function for a phase, so both wrote their own walk.
--
-- `granularity` makes the level an argument. The journey is one
-- parent-child walk — phase > scenario > path > step/layer > cell — so
-- the level belongs in a PARAMETER, not in five more function names.
--
-- Defaults to array['cell'], so every v2 caller keeps v2 behaviour
-- byte-for-byte: same rows, same order, same total_matched.
--
-- Retrieval per kind, and why they differ:
--   cell        full fusion — vector + prose keyword + breadcrumb keyword
--   structural  keyword on the NAME only
-- A phase has a name and nothing else to embed; there is no prose body and
-- no chunk in corpus_chunks. Running it through the vector arm would score
-- it against an embedding that does not exist. Keyword on the name is not a
-- lesser retriever here, it is the whole of what the row is.
--
-- total_matched counts rows across the REQUESTED granularities, so
-- "22 of 22 scenarios" is as honest as "113 cells mention Zoom".
...
begin
  ...
  select
    e.knd, e.rid, e.ttl, e.snip, e.descr, e.lane, e.step_name,
    e.scen, e.ph_name, e.path_name, e.lnk, e.upd, e.sim, e.score, e.how,
    total
  from everything e
  order by e.score desc nulls last,
           e.ord nulls last,
           e.ph_ord, e.sc_ord, e.path_name nulls first, e.slot, e.lrow, e.rid
  limit cap;
end;
$function$;

revoke all on function public.search_blueprint(text, extensions.vector, int, text, int, text, text, text, text, text[]) from public;
grant execute on function public.search_blueprint(text, extensions.vector, int, text, int, text, text, text, text, text[])
