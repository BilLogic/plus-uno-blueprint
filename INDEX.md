...
| docs/adr/0017-a-check-that-cannot-see-its-subject-says-so.md | developers | Each live check is answered on its own — publishable credentials run on every pull request, privileged ones run nightly and never on a pull request, one stays manual — and whatever the answer, a job that did not verify something says which thing, on every pull request. |
