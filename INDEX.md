...
| docs/adr/0010-the-canvas-and-the-shell-run-on-separate-clocks.md | developers | The shell's entrance stagger and the canvas's reveal ladder are two clocks on purpose, joined by reads that run one way only; a surface that owns its own query takes its own hold session, and still waits for the lane it arrives beside. |
| docs/adr/0011-featured-is-one-column-two-verbs.md | developers | A resource's `featured` flag is one boolean whose meaning depends on the resource's kind — a featured attachment is the owner's preview, a featured link is one of its buttons — rather than two columns or a role enum, because the two verbs are what a reader sees and the one column is what an author decides. |
| docs/adr/0012-uno-is-a-deployment-of-the-template.md | developers | The template `agentic-service-blueprinting` is the canonical application code, and uno becomes a deployment of it — the same code plus a data, environment, and content overlay carrying PLUS's branding and blueprint — rather than a fork maintained in parallel, because the generic thing must be the base and PLUS is already almost entirely data. |
