...
| `Zoom/Pencil` | 79 | The session's videoconference. One pill, because a session runs on one or the other and the blueprint does not branch on which. |
...
| `Zoom` | 4 | Zoom used outside a tutoring session — an info session, a group interview. |
