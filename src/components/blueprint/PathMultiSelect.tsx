 ...
 * ONE COLUMN PER KIND.
 *
 * This was two `Set`s of kinds, each listing a member twice — the residue of
 * `20260830190000` folding `alternative` onto `variant` and `unhappy` onto
 * `exception`, which turned two distinct entries into the same one. A `Set`
 * absorbs a repeat rather than failing, so the fold left a pair of literals
 * that read as nonsense and behaved correctly, and nothing could say so.
 *
 * A total map has the properties the pair of sets lacked. Every kind is
 * assigned, because `Record<PathKind, …>` is exhaustive; and each is assigned
 * once, because a repeated key is a syntax the type system rejects. The
 * template arrived at the same shape from the other direction, where the
 * overlap was real and drew a variant path twice.
 */
const PATH_COLUMN_BY_KIND: Record<PathKind, AssignedPathColumn> = {
  happy: 'primary',
...
export function PathMultiSelect({
  ...
  // horizontal picker splits the kinds into side-by-side columns.
  const columns =
