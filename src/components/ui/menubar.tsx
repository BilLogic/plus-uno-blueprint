...
/*
 * Menu rows are `text-sm`, matching `dropdown-menu.tsx` (see the note there):
 * UI text sits on `sm`; labels and shortcuts stay `xs` chrome. Three
 * families sharing one item vocabulary must not disagree about row size.
 */
function MenubarContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn("min-w-36 rounded-lg bg-popover p-1 text-popover-foreground shadow-md ring-1 ring-border-overlay duration-100 data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95", className )}
      {...props}
    />
  )
}
