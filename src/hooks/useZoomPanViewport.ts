...
/**
 * The zoom a MOVING camera should decide its semantic tier from: the lower of
 * where it is leaving and where it is going.
 *
 * The tier is a rendering decision — cells as text, or cells as flat blocks —
 * and a camera in motion should make it once for the whole journey: never
 * mid-flight, and never differently depending on which way it is travelling.
 * Reading the DESTINATION alone did the first and not the second, which is
 * where zoom-in and zoom-out feeling like different animations came from:
 *
 *   zooming IN  (blocks → text) stamped the text tier on frame one, so every
 *     frame of the ease rasterized the whole board's text at a scale that
 *     changes each frame — the most expensive thing this canvas can draw,
 *     sustained across the entire glide.
 *   zooming OUT (text → blocks) stamped blocks on frame one, so the detail
 *     vanished before the camera had moved at all: a cut, not a transition.
 *
 * Taking the minimum renders the cheap tier in both directions while the board
 * is in motion, and lets the destination tier land when it stops. Detail
 * resolves as the camera arrives, which is what every map does, and the two
 * directions become symmetric because they now animate the same thing.
 *
 * Deliberately unchanged: when both ends sit on the same side of the
 * threshold the minimum is on that side too, so a transition that never
 * crosses is exactly what it was — which covers overview→phase, the case that
 * already felt right.
 */
...
function applyTransformToElement(
   ...
   * threshold mid-flight and the whole board changed tier on whichever frame
   * happened to cross it — a style/paint burst in the busiest stretch of the
   * animation, felt as the glide glitching. That much was fixed by reading
   * the destination instead. What the destination alone could not fix is
   * that it made the two directions behave differently, which is the second
