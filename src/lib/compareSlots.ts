...
/**
 * The fold pin rule: a shared cell with a direct (one-hop) trigger/needs
 * edge to a non-shared cell is context a reviewer needs while reading
 * that divergence — its column must not fold. One hop by design:
 * transitive closure can pin the whole board and defeat compression.
 */
export function computePinnedColumns(
  ...
  const statusByCellId = new Map<string, CompareStatus>()
  for (const slot of model.slots) {
    for (const entry of Object.values(slot.perPath)) {
      ...
      for (const cellId of entry.cellIds) {
        ...
        statusByCellId.set(cellId, slot.verdict)
      }
    }
  }
  ...
  for (const blueprint of blueprints) {
    for (const trigger of blueprint.triggers) {
      const sourceStatus = statusByCellId.get(trigger.source_cell_id)
      const targetStatus = statusByCellId.get(trigger.target_cell_id)
      if (sourceStatus === undefined || targetStatus === undefined) continue
      if (sourceStatus === 'shared' && targetStatus !== 'shared') {
        const column = columnByCellId.get(trigger.source_cell_id)
        if (column) pinned.add(column)
      }
      if (targetStatus === 'shared' && sourceStatus !== 'shared') {
        const column = columnByCellId.get(trigger.target_cell_id)
        if (column) pinned.add(column)
      }
    }
  }
