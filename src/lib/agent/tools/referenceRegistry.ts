 ...
 *
 * `referenceDocs.ts` owns the documents this template ships and names their paths,
 ...
 * respell the template's paths (the template's own copies ship inside the package it
 ...
 * serve its own copy of one of the template's. Both are operations on a record, so
...
/**
 * Serve these documents alongside the template's own.
 *
 * A key the template already serves REPLACES that document and adds no name; a key
