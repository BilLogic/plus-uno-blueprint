...
| Work across several services | a deployment may hold more than one. The reads that take a `service` filter (`list_blueprint`, `list_stakeholders`, `search_blueprint`) cover every service in the deployment when it is omitted — the default is the whole deployment, not the board on screen; pass a service name to narrow to it, or "all" to say the default out loud. Writes always land on the service the human is looking at |
