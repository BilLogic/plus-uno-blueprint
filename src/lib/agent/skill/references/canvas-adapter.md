...
| Work across several services | a deployment may hold more than one. The reads that take a `service` filter (`list_scenarios`, `list_stakeholders`) default to the service on screen; pass a service name to narrow, or "all" to reach across the deployment. Writes always land on the service the human is looking at |
