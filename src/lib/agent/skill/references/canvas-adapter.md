...
| Run an audit (`/sb:audit`) | FULLY LIVE: same roster and check docs (`read_reference`), checks run over `get_blueprint` reads, findings land as rows via `record_finding` (dedupe is built in: open updates in place, dismissed stays dismissed, resolved reopens). Reuse the run_id the first `record_finding` returns for the whole run. Triage = `set_finding_status`; the ledger = `list_findings`. Canvas findings identify cells by id (cell_keys are written as the ids), so canvas and IDE fingerprints are separate dedupe spaces |
