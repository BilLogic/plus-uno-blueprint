 ...
 *      every dimmed neighbour through the row's `Math.max` — six untouched
 *      panels once grew from 2218px to 4250px each, rendering two thousand
 *      pixels of empty gray apiece. Note the trigger: the expansion, not
 *      the focus. Gating on focus alone made rule 2 unsatisfiable, since
 *      excluding a panel is itself a change to the row height.
 *
...
export function resolveScenarioPanelHeight({
  ...
  /** The row's shared height — computed WITHOUT the focused scenario. */
  rowPanelHeight: number | undefined
  /** The focused scenario's own estimated height. */
  ownHeightFloor: number | undefined
  isFocused: boolean
}): number | undefined {
  if (!isFocused || ownHeightFloor === undefined) return rowPanelHeight
