...
export type CellTouchpoint = {
   ...
   * has none. A property of the touchpoint the deployment owns, authored once
   * and read off the row (#326) — never a tool name matched against a table
   * baked into the renderer. Optional so a fallback board, which has no
   * registry, simply omits it.
   */
  iconUrl?: string | null
  summary: string | null
  /**
