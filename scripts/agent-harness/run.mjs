    ...
    switch (name) {
      ...
      case 'list_cell_dependencies': {
        ...
        record.result = rows?.length
          ? [`${rows.length} link(s):`, ...rows.map((e) => `${e.source_cell_id} --${e.kind ?? 'sets_off'}--> ${e.target_cell_id}${e.label ? ` "${e.label}"` : ''} (${e.id})`)].join('\n')
      ...
      case 'focus_cell': record.result = 'Focused the active canvas camera on the cell.'; return record.result
      default:
